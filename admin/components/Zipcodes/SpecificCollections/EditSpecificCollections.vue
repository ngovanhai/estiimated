<template>
  <el-dialog title="Collection Rule" :visible.sync="isOpen" v-loading="isSaving">
    <ul class="panel-component-items">
      <li class="panel-component-item">
        <div class="panel-component-item-title">Collections</div>
    ...
    isOpen: function(val) {
      if (val) {
        if (this.currentRule) {
          ...
          if (this.rule.collection_id) {
            let collection = this.collections.find(
              e => e.id == this.rule.collection_id
            );
  ...
  methods: {
    ...
    checkRule() {
      if (this.selectedCollections.length == 0) {
        return "You must select one collection";
      }
    ...
    onSave() {
      ...
      if (error) {
        ...
        this.$http
          .post(
            window.zipcodeSpecificCollectionsApi,
            {
              rule: rule,
              listCollections: this.selectedCollections,
